...
const Box = dynamic(() => import('@react-three/drei').then((m) => m.Box), { ssr: false })
import Link from 'next/link'
import { FaGithub, FaMagic, FaBolt, FaTools } from 'react-icons/fa'
import { useState } from 'react'
import dynamic from 'next/dynamic'
const InnovationCube = dynamic(() => import('@/components/three/InnovationCube'), { ssr: false })
  ...
  return (
    <section id="projects" className="section-padding bg-gradient-dark relative overflow-hidden">
      <div className="container-custom mx-auto">
        ...
        >
          {/* Client-only 3D Canvas to avoid SSR issues */}
          <InnovationCube />
        </motion.div>
