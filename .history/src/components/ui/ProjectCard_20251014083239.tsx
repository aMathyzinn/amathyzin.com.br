...
export function ProjectCard({ title, description, videoUrl, projectUrl, tags = [], category, size }: ProjectCardProps) {
  return (
    <motion.div
      whileHover={{ y: -8 }}
      className="card-gradient group h-full flex flex-col border border-white/10 hover:border-primary/40 transition-colors rounded-2xl overflow-hidden"
    >
      {/* Video Preview */}
          ...
          <Image
            src={
              videoUrl.includes('youtube')
                ...
                : 'https://i.ytimg.com/vi/Pxcaxgx_j-0/hqdefault.jpg'
            }
            alt={`${title} thumbnail`}
