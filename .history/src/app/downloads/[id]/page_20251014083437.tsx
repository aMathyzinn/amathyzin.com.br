...
export default async function ProductPage({ params }: Params) {
  ...
  const videoId = product?.videoUrl?.includes('youtube')
    ? (product.videoUrl.includes('/embed/')
        ? product.videoUrl.split('/embed/')[1].split('?')[0]
        : product.videoUrl.split('v=')[1]?.split('&')[0] || product.videoUrl.split('/').pop())
    : undefined
  const thumbSrc = videoId ? `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg` : (product?.imageUrl || `/img/${id}.png`)
    ...
    <div className="min-h-screen pt-20 bg-gradient-dark">
      ...
      <div className="container-custom mx-auto px-4 py-8">
        <div className="space-y-6">
          {/* Tutorial Completo */}
            <div className="card-gradient rounded-2xl p-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <a href={product.videoUrl} target="_blank" rel="noopener noreferrer" className="btn-primary">
          ...
          {/* Download Card */}
          <div className="card-gradient rounded-2xl p-6 border border-white/10">
            ...
            <div className="rounded-full bg-dark/40 overflow-hidden mb-4">
              <div className="h-2 w-full bg-gradient-to-r from-primary to-secondary" />
            </div>

            <label className="flex items-center gap-3 text-white/80 mb-4 cursor-pointer">
              <input type="checkbox" className="w-4 h-4 text-primary bg-dark border-primary/30 rounded focus:ring-primary" />
              <span className="text-sm">Concordo com os Termos de Serviço</span>
            </label>

            <a href={`/downloads/files/${product.fileName}`} className="btn-primary w-full justify-center shine-effect group mb-3">
          ...
          <div className="card-gradient rounded-2xl p-6">
              <h4 className="text-lg font-semibold mb-3 flex items-center gap-2"><FaShieldAlt /> Segurança</h4>
          ...
          <div className="card-gradient rounded-2xl p-6">
              <h4 className="text-lg font-semibold mb-3">Dependências</h4>
              <div className="space-y-2">
                <a href="#" className="btn-secondary w-full justify-center group"><FaExternalLinkAlt /> BloxStrap</a>
                <a href="#" className="btn-secondary w-full justify-center group"><FaExternalLinkAlt /> .NET Framework</a>
              </div>
            </div>

          <div className="card-gradient rounded-2xl p-6">
              <h4 className="text-lg font-semibold mb-2 flex items-center gap-2"><FaGithub /> Código Fonte</h4>
              <p className="text-white/60 text-sm mb-3">Veja o código no GitHub e contribua com melhorias.</p>
              <a href="#" className="btn-primary w-full justify-center group"><FaGithub /> Ver no GitHub</a>
            </div>
        </div>
      </div>
      ...
      <div className="container-custom mx-auto px-4 pb-12">
        <div className="card-gradient rounded-2xl p-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <a href="/youtube" className="btn-secondary"><FaYoutube /> Canal do YouTube</a>
